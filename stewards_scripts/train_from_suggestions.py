"""Train a sidewalk fix model from polygon suggestions and produce corrected polygons.

Usage
-----
python train_from_suggestions.py \
    --geojson suggestions.geojson \
    --tiles_dir .../tiles \
    --t2n_dir .../masks_tile2net_polygons \
    --conf_dir .../masks_confidence \
    --output corrected_polygons.geojson \
...
def find_covered_tile_ids(gdf, zoom, tiles_dir, t2n_dir, conf_dir):
    """Find all zoom-level tile IDs that intersect the polygons in gdf.

    Only returns tiles for which RGB, T2N mask, and confidence files exist.
    """
    minlon, minlat, maxlon, maxlat = gdf.total_bounds
    x_min, y_max = deg2num(minlon, minlat, zoom)
    x_max, y_min = deg2num(maxlon, maxlat, zoom)
    ...
    tile_ids = []
    for x in tqdm(range(x_min, x_max + 1), desc="Scanning tile grid"):
        for y in range(y_min, y_max + 1):
            ...
            if not (conf_dir / f"{tid}.png").exists():
                continue
            # Check if any polygon falls in this tile
            w, s, e, n = get_tile_bounds(x, y, zoom)
            clipped = gdf.cx[w:e, s:n]
            if len(clipped) > 0:
                tile_ids.append(tid)
...
# ─────────────────────────────────────────────────────────────────────────────
# Rasterize suggestions → GT masks
# ─────────────────────────────────────────────────────────────────────────────
...
def rasterize_suggestions_to_gt(gdf, tile_ids, gt_dir):
    ...
    for tid in tqdm(tile_ids, desc="Rasterizing suggestions"):
        ...
        mask = rasterize_polygons_to_mask(gdf, x, y, zoom=19, resolution=256)
...
def main():
    ...
    print(f"\nRunning inference on {len(tile_ids)} tiles...")
    predictions = run_inference(
        model, tile_ids, device, args.head,
        tiles_dir=args.tiles_dir,
        t2n_dir=args.t2n_dir,
        conf_dir=args.conf_dir,
    )
    ...
    # ── Step 8: Re-polygonize → output GeoJSON ──
    print("\nRe-polygonizing predicted masks...")
    gdf_out = polygonize_predictions(predictions)

    output_path = Path(args.output)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    gdf_out.to_file(output_path, driver="GeoJSON")
    print(f"\nOutput saved to: {output_path}")
    print(f"  {len(gdf_out)} polygons from {gdf_out['tile_id'].nunique()} zoom-18 tiles")
